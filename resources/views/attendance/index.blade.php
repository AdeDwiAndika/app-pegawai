...
<div class="max-w-6xl mx-auto p-6">
    ...
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-semibold text-gray-800">Daftar Absensi</h1>
        ...
        <table class="min-w-full text-sm text-gray-700">
            ...
            <tbody>
                ...
                <tr class="hover:bg-gray-50">
                    <td class="py-3 px-4 text-center space-x-3">{{ $atten->id }}</td>
                    <td class="py-3 px-4 text-center space-x-3">{{ $atten->employee->nama_lengkap ?? '-' }}</td>
                    <td class="py-3 px-4 text-center space-x-3">{{ $atten->tanggal }}</td>
                    <td class="py-3 px-4 text-center space-x-3">{{ $atten->waktu_masuk }}</td>
                    <td class="py-3 px-4 text-center space-x-3">{{ $atten->waktu_keluar }}</td>
                    <td class="py-3 px-4 text-center space-x-3">{{ $atten->status_absensi }}</td>
                    <td class="py-3 px-4 text-center space-x-3">
                        ...
                        <form action="{{ route('attendances.destroy', $atten->id) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="6" class="px-4 py-4 text-center text-gray-500">Belum ada data absensi</td>
                </tr>
                @endforelse
            </tbody>
        </table>
