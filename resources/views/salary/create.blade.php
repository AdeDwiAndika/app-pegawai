...
<div class="max-w-3xl mx-auto mt-10 bg-white border border-gray-200 rounded-[24px] w-full p-6 relative">
    <div class="flex justify-between items-start mb-6">
        <div>
            <h1 class="text-2xl font-semibold text-gray-800">Tambah Data Gaji Pegawai</h1>
            <p class="text-gray-500 w-3/4">Lengkapi form dibawah untuk menambah data gaji pegawai</p>
        </div>
    ...
    <form action="{{ route('salaries.store') }}" method="POST" class="space-y-3">
        @csrf
        ...
        <div>
            ...
            <input type="text" name="bulan"
                ...
                placeholder="Misal: Oktober" required>
        </div>

        <div>
            <label class="block text-gray-700 mb-2">Gaji Pokok</label>
        ...
        <div>
            <label class="block text-gray-700 mb-2">Tunjangan</label>
            <input type="number" step="0.01" name="tunjangan"
                class="w-full border border-gray-300 rounded-[16px] px-4 py-2 focus:outline-none focus:ring-1 focus:ring-green-500">
        </div>

        <div>
            <label class="block text-gray-700 mb-2">Potongan</label>
